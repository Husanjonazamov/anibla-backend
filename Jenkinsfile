pipeline {
    ...
    environment {
        PROD_ENV     = "/opt/env/.env.backend"
        IMAGE_NAME   = "backend"
        TEST_TAG     = "test"
        PROD_TAG     = "latest"
        CONTAINER_DB = "backend_db_test"
        CONTAINER_WEB = "backend_web_test"
        CONTAINER_REDIS = "backend_redis_test"
        STACK_NAME = "backend"
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', credentialsId: 'ssh', url: 'git@github.com:JscorpTech/backend.git'
            }
        }
        stage('Build Image') {
            steps {
